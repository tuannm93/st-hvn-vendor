...
class ProgImport20 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:prog_import20 {args?*}';
    ...
    public function handle()
    {
        try {
            DB::beginTransaction();
            $this->logService->log('進捗表管理インポートシェル（20日締め）start');
            $this->logService->log('進捗表管理インポートファイル作成start');
            $argument = $this->argument();
            $importFile = $this->progImportService->insertProgImportFiles($argument, $flag = true);
            $this->logService->log('進捗表管理インポートファイル作成end');
            $this->logService->log('案件情報の取得start');
            $commissionInfos = $this->progImportService->findCommissionInfos($flag = true);
            ...
            $this->logService->log('進捗表管理インポートシェル（20日締め）end');
            DB::commit();
