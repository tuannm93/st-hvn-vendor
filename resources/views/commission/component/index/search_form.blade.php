...
<input type="hidden" value="{{ isset($params['affiliationId']) ? $params['affiliationId'] : '' }}" name="affiliationId">
<div class="search-block p-4">
    <div class="row">
        ...
        <div class="col-md-6 mb-2">
            <div class="form-inline">
                ...
                <div class="col-md-8">
                    {{Form::text('customer_tel',($lastSearchData !== null) ? (isset($lastSearchData['customer_tel']) ? $lastSearchData['customer_tel'] : null) : null,['class'=>'form-control w-100', 'data-rule-number' => 'true'])}}
                </div>
        ...
        <div class="col-md-6 mb-2">
            <div class="form-inline">
                <div class="col-md-4">
                    <label for="">
                        @lang("commissioninfos.lbl.site_id")
                    </label>
                </div>
                <div class="col-md-8">
                    {{Form::select('site_id[]', $siteList, ($lastSearchData !== null) ? (isset($lastSearchData['site_id']) ? $lastSearchData['site_id'] : null) : null, ['id' => 'site_id', 'multiple' => true, 'class' => ' form-control'])}}
                </div>

            </div>
            @if($errors->has('site_id'))
                <small class="text-danger offset-md-4 col-md-8 ">{{$errors->first('site_id')}}</small>
            @endif
        </div>
        <div class="col-md-6 mb-2">
    ...
    <div class="search_btn_block text-center">
        {{Form::button(trans('commissioninfos.lbl.btn_search'),['name'=>'search','class'=>'btn_orange_m btnOver btnSearch btn btn--gradient-orange'])}}
        @if((\Auth::user()->auth == 'system' || \Auth::user()->auth == 'admin'|| \Auth::user()->auth == 'accounting_admin'))
             {{ Form::submit(trans('commissioninfos.lbl.btn_csv'), ['class' => 'btn_orange_m btnOver btn btn--gradient-orange ml-3 d-none', 'name' => 'csv_out', 'id' => 'btnExport']) }}
        @endif
    </div>
</div>
{!! Form::close() !!}
