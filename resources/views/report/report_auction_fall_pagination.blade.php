@if ($paginator->hasPages())
    <div class="dataTables_paginate">
        ...
        @else
            <a data-url="{{ $paginator->previousPageUrl() }}" class="paginate_button previous active" rel="prev"
               aria-controls="tbBillSearch" id="tbBillSearch_previous">&lt; @lang('mcorp_list.prev')</a>
        @endif
        <span class="pl-3 pr-3"></span>
        @if ($paginator->hasMorePages())
            <a data-url="{{ $paginator->nextPageUrl() }}" class="paginate_button next active" rel="next"
