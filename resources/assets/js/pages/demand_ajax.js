var ajaxCommon = (function() {
            ...
            success: function(data) {
                if (data.length) {
                    ...
                    $(controlEl.resultArea).html(data).promise().done(function(){
                        var showCSV = jQuery('#table-demandlist').attr('data-csvshow');
                        if (showCSV == 1) {
                            if (jQuery('#demand_csv').hasClass('d-none')){
                                jQuery('#demand_csv').removeClass('d-none');
                            }
                        } else {
                            if (!jQuery('#demand_csv').hasClass('d-none'))
                            jQuery('#demand_csv').addClass('d-none');
                        }
                        //auto fill value
                        var corpName = jQuery('#table-demandlist').attr('data-corp-name');
                        var corpNameKana = jQuery('#table-demandlist').attr('data-corp-name-kana');
                        var corpId = jQuery('#table-demandlist').attr('data-corp-id');
                        if ( !jQuery('#corp_name').val() ){
                            jQuery('#corp_name').val(corpName);
                        }
                        if ( !jQuery('#corp_name_kana').val() ){
                            jQuery('#corp_name_kana').val(corpNameKana);
                        }
                        if ( !jQuery('#corp_id').val() ){
                            jQuery('#corp_id').val(corpId);
                        }
                    });
    ...
    var search = function(url, controlEl, triggerSearch) {
        var sorted = {};
        var page = 0;
        //load current page
        ...
        if (triggerSearch == 1) {
            $(controlEl.searchEl).trigger('click');
        }
