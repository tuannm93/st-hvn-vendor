var demandModule = (function () {
    ...
    var $siteEl = $('#site_id'),
        ...
        $lat = $('#latitude'),
        $lng = $('#longitude'),
        $statusId = $('#demand_status_id');

    var $modalPopup = $('#modal-popup'),
        $modalDialog = $('#modal-dialog'),
        $dropdownDemandDetail = $('#dropdown-detail');
    ...
    var demandObj = {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if(typeof res.code !== "undefined" && res.code === 1){
                        ...
                        $totalCurrentViewSection.html('<span class="text-danger">' + res.data || 0 + '</span>');
                    }else{
                        var currentView = $totalCurrentViewSection.find('span').html();
                        $totalCurrentViewSection.html('<span class="text-danger">' + currentView + '</span>');
                    }
                },
                error: function (err) {
                    console.log(err);
        ...
        submitUploadAttachFile: function (btn, url) {
            $(btn).parents('form').find('textarea').data('rule-required', false);
        ...
        resetAttachedFileValue: function (btnReset) {
            ...
            parent.find('.reset-file-name').html(noFileChoosen);
        },
        ...
        changeSiteId: function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var val = demandGenreId;
                    var val = !copy ? demandGenreId : '';

                    var genreHtml = demandObj.renderHtml(genresList, true, val);
                    $genreEl.html(genreHtml);
                    $genreEl.trigger('change');
                    initTime();
        ...
        getCategoryList: function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var categoryHtml = demandObj.renderHtml(categories, true, categoryId);
        ...
        getCategory: function (genreId = null, el = $categoryEl, firstLoad = true) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    var categoryHtml;
                    if (firstLoad) {
                        categoryHtml = demandObj.renderHtml(categories, true, parseInt(categoryId));
                    } else {
                        categoryHtml = demandObj.renderHtml(categories, true, '');
                    }
        ...
        getInquiryItem: function (genreId, displayContent = true) {
            if(!cross){
                genreId = $('#before_demandinfo_genre_id').val();
            }
            $.ajax({
                type: 'GET',
                ...
                success: function (res) {
                    var genreName = $('#genre_id option:selected').text();
                    var content = '';
                    if (!hasChangeDemandContent) {
                        if (cross) {
                            content += 'クロスセル案件番号：';
                            content += id;
                            content += ' ジャンル：';
                            content += genreName;
                            content += ' 同じ加盟店を希望or違う加盟店でもよい';
                            content += res.data ? res.data.inquiry_item : '';
                            oldDemandContent = content;

                        } else {
                            content = $('#demand_contents').val();
                        }

                        if (validateFail) {
                            $demandContentEl.val(oldDemandContent);
                        } else {
                            $demandContentEl.val(content);
                        }
        ...
        changeGenreSelection: function (genreId, renderElm = $categoryEl, displayContent = true, firstLoad = true) {
            ...
            if (copy) {
                demandObj.getInquiryItem(defaultCrossGenreId, displayContent);
            }
        ...
        enableTextBox: function (radioElm, enableElm, disabledElm) {
            if ($(radioElm).is(':checked')) {
                ...
                parent.find('.' + enableElm).attr('value', '').attr('disabled', false);
                parent.find('.' + disabledElm).attr('value', '').attr('disabled', true);
            }
        },
        getSelectionSystemList: function (genreId, address1, hasDefault = false) {
            if($genreEl.val() != '') {
                $.ajax({
                    type: 'GET',
                    ...
                    success: function (res) {
                        ...
                        var defaultValue = hasDefault ? (selectionSystemValue ? selectionSystemValue : demandSelectionSystem) : res.data.default_value;
        ...
        existsAutoCommissionCorp: function () {
            ...
            if (siteId != "" && categoryId != "" && genreId != "" && address1 != "") {
                $.ajax({
                    ...
                    data: {genre_id: genreId, prefecture_code: address1, category_id: categoryId, site_id: siteId},
                    xhr: function () {
                        return progress.createXHR();
                    },
                    beforeSend: function () {
        ...
        controlContactTimeEntryFields: function () {
            var turnOnOff = function turnOnOff(flg) {
                var option = {
                    controlType: 'select',
                    oneLine: true,
                    timeText: '時間',
                ...
                if (flg) {
                    $("#DemandInfoContactDesiredTimeFrom, #DemandInfoContactDesiredTimeTo").prop('readonly', true).val("").addClass("readonly").datetimepicker("destroy");
                    $("#DemandInfoContactDesiredTimeFrom").val('');
                    $("#DemandInfoContactDesiredTimeTo").val('');
                    $contactDesiredTimeEl.prop('readonly', false).removeClass("readonly").datetimepicker(option);
                } else {
                    $("#DemandInfoContactDesiredTimeFrom, #DemandInfoContactDesiredTimeTo").prop('readonly', false).removeClass("readonly").datetimepicker(option);
                    $contactDesiredTimeEl.val("");
                    $contactDesiredTimeEl.prop('disabled', true).prop('readonly', true).addClass("readonly").datetimepicker("destroy");
                }
            };
            if ($("#is_contact_time_range_flg_0").is(':checked')) {
                turnOnOff(true);
                return;
            }
            if ($("#is_contact_time_range_flg_1").is(':checked')) {
                turnOnOff(false);
                return;
            }
        },
        ...
        getCrossSellSiteSelection: function (siteId, hasDefaultValue = false) {
            if (siteId != "") {
                $.ajax({
                    type: 'GET',
                    ...
                    success: function (res) {
                        var defaultValue = hasDefaultValue ? crossSellSourceGenre : '';

                        var crossSourceSiteHtml = demandObj.renderHtml(res.data.category_list, false, defaultValue);
    ...
    var detectCommissionMaxLimit = {
        ...
        showLimitNum: function (selectionVal) {
            if (selectionVal == '0' || selectionVal == '4') {
                ...
                $('#manual_selection_limit').val(detectCommissionMaxLimit.limit.manual_selection_limit);
            }
            if (selectionVal == '2' || selectionVal == '3') {
                ...
                $('#auction_selection_limit').val(detectCommissionMaxLimit.limit.auction_selection_limit);
            }
        },
    ...
    var loadModal = function (url_display_commission, modal = $modalPopup) {
        var modal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : $modalPopup;
        var key = arguments.length > 1 && arguments[2] !== undefined ? arguments[2] : 1;
        $.ajax({
            type: "GET",
            ...
            success: function (data) {
                modal.children().children().find('.modal-header').find('span#modal-title').html('取次先'+ key + '情報');
                modal.children().children().find('.modal-body').html(data);
                modal.modal('show');
            },
            error: function () {
                console.log("Error!");
    ...
    function loadCommissionInfo(commissions, currentIndex = 1) {
    ...
    function loadDropdown(url_display_commission, dropdown = $dropdownDemandDetail, dropdown_position) {
        var dropdown_position = arguments.length > 1 && arguments[3] !== undefined ? arguments[3] : null;
        var key = arguments.length > 1 && arguments[2] !== undefined ? arguments[2] : 1;
        $.ajax({
            type: "GET",
            url: url_display_commission,
            ...
            success: function (data) {
                dropdown.find('span#dropdown-title').html('取次先'+ key + '情報');
    ...
    var init = function () {
        ...
        var genreId = $genreEl.val();
        demandObj.changeGenreSelection(genreId, $categoryEl, false, true);
        ...
        var siteId = $siteEl.val();
        if (siteId) {
            demandObj.getCommissionMaxLimit(siteId);
        }
        ...
        $visitTimeDiv.not(':first').find('input[type="text"]').on('change', function () {
            $visitTimeDiv.find('input[type="text"]').addClass('ignore');
        ...
        $genreEl.on('change', function () {
            ...
            var address1 = $address1El.val();
            demandObj.getBusinessTripAmount(genreId, address1);
            demandObj.changeGenreSelection(genreId, $categoryEl, true, false);
            demandObj.getSelectionSystemList(genreId, address1, false);
            demandObj.existsAutoCommissionCorp();
        ...
        //Demand Detail
        var $resetAttachFile = $('.reset-default-attached-file'),
            $chosenAttachFile = $('.chosen-attach-file'),
            $submitAttachFile = $('#submit-form-attached-file'),
            $deleteAttachFile = $('.btn-delete-attached-file');

        $deleteAttachFile.on('click', function (e) {
            e.preventDefault();
            demandObj.deleteAttachedFile($(this), apiRoutes.deleteAttachFileUrl);
        });
        $chosenAttachFile.on('click', function (e) {
            e.preventDefault();
            demandObj.triggerFile($(this).parents('.trigger-section-file'));
        });
        $resetAttachFile.on('click', function (e) {
            e.preventDefault();
            demandObj.resetAttachedFileValue($(this));
        });
        ...
        $submitAttachFile.on('click', function (e) {
            e.preventDefault();
            demandObj.submitUploadAttachFile(this, apiRoutes.uploadAttachedFileUrl);
        });
        // demandObj.getInquiryItem($genreEl.val());
        $demandStatusEl.trigger('change');

        // $crossSellSourceSiteEl.trigger('change');
        var siteId = $crossSellSourceSiteEl.val();
        demandObj.getCrossSellSiteSelection(siteId, true);
        var time = 0;
        var timer = setInterval(function () {
            demandObj.writeBrowse();
            if (++time > 900) clearTimeout(timer);
        }, 5000);

        var time2 = 0;
        var writeTime = setInterval(function () {
            demandObj.countBrowse();
            if (++time2 > 900) clearTimeout(writeTime);
        }, 5000);
        // $siteEl.trigger('change');
        // $genreEl.trigger('change');
        // demandObj.getSelectionSystemList($genreEl.val(), $address1El.val(), true);
        ...
        $('#customControlInline').on('click', function(){
            $('#send_commission_info_btn').toggle();
        });
    };
    ...
    if (validateFail) {
        ...
        demandObj.getSelectionSystemList(genreId, address1, true);
        demandObj.getCommissionMaxLimit($siteEl.val());
        demandObj.existsAutoCommissionCorp();

        var crossSellSourceSite = $crossSellSourceSiteEl.val();
        demandObj.getCrossSellSiteSelection(crossSellSourceSite, true);

    }


    $sourceDemandId.on('change', function(e){
        e.preventDefault();
        if(validateFail){
            $(this).parent().find($('.invalid-feedback')).remove();
        }
    ...
    $(document).ready(function() {
        $('.m-corps-detail').on('click', function (e) {
            e.preventDefault();
            var dropdown_position = $(this).offset().top + $(this).outerHeight();
            var urlData = $(this).data('url_data') + '?fee_data=' + $(this).parents('.form-table').find('.get-fee-data').text();
            loadDropdown(urlData, $dropdownDemandDetail, $(this).data('key'), dropdown_position);
        });
    });
