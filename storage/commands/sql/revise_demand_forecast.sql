...
from (
	...
	(
		select
			...
			state_id,
			case 
				when trunc(avg(wind_speed_level)) = 4 then 2 
				when trunc(avg(wind_speed_level)) = 3 then 2
				when trunc(avg(wind_speed_level)) = 2 then 1 
				when trunc(avg(wind_speed_level)) = 1 then 2 
			end wl
		from
			weather_forecasts
		group by
...
and demand_forecasts.demand_count0_min = 0 
and demand_forecasts.demand_count0_max = 0 
and demand_forecasts.demand_count1_min = 0 
and demand_forecasts.demand_count1_max = 0 
and demand_forecasts.demand_count2_min = 0 
and demand_forecasts.demand_count2_max = 0
;